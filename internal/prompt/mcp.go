...
const (
	MCPSystemPrompt = `You are an AI assistant with access to a Model Context Protocol (MCP) server.
...
Remember to clearly explain your reasoning and actions to the user.`
	...
	MCPNextStepPrompt = `Based on the current state and available tools, what should be done next?
...
If you've already made progress, consider what additional information you need or what actions would move you closer to completing the task.`
	...
	MCPToolErrorPrompt = `You encountered an error with the tool '%s'.
...
Please check the tool specifications and try again with corrected parameters.`
	...
	MCPMultimediaResponsePrompt = `You've received a multimedia response (image, audio, etc.) from the tool '%s'.
